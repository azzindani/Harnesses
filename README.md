...
- `IDLE_EXEMPT` — harness types the idle sweep must *never* stop, e.g. `IDLE_EXEMPT=claude,opencode`. Empty by default, but worth setting for a harness you use daily, because nearly all of its RAM is the CLI rather than the container: a claude harness holds 38MB with its sessions closed against 984MB with three of them running. `SESSION_IDLE_HOURS` then reclaims about as much as a stop would, while keeping the container means no cold start on the next visit and nothing for `docker container prune` to remove — that command deletes stopped containers, which is what an idle-stopped harness is.
...
- A session nobody has had a tab open on for `SESSION_IDLE_HOURS` (default `24`, per-harness override as `24,opencode=3`) is closed on its own — its CLI process and its `ttyd` — while `main` and every other session keep running, even on an `IDLE_EXEMPT` harness. Each slug is a whole CLI (~1GB for opencode), and idle-stop (below) can't reclaim one while anything else in the container is in use. Visiting the URL again starts it fresh; the conversation is still in `history/`.
- On an `IDLE_EXEMPT` harness the same clock also applies to `main`: its CLI is stopped (never its `ttyd`, which is that container's `PID 1`), because the container it sits in is never stopped to reclaim it. Reconnecting relaunches with `--continue`, so the conversation comes back.
...
Both writable spots are visible to agents with no restart and no extra mount: `data/` is already `/workspace/data` in every harness container, and `project/uploads` is `/workspace/uploads`. That makes this the way to hand a file to an agent from a phone, or from any machine with no shell on the box. Files you upload are *read* by agents — treat them as input those agents will act on.

⚠️ **`data/` is also the sweep's exchange.** `scripts/sweep/run_all_checks.sh` clears everything at the top level of `data/` that isn't in its `FIXTURES` allowlist, so a file dropped loose in `data/` is gone after the next round. `data/uploads/` **is** in that allowlist and survives — put anything you want to keep there. Nothing ever prunes it automatically, so it grows until you empty it (`du -sh data/uploads` when the exchange looks larger than the fixtures explain). `scripts/sweep/verify_artifacts.py` also skips `uploads` when it walks the tree, so your own files aren't graded as a checker's output.

Like `ttyd` itself, `dufs` has no login of its own here — Caddy's `forward_auth` already gates every request before it reaches the container, so anyone with a valid `harness_session` cookie or a `files`-scoped token gets read access to all three directories plus write access to the drop box. Note what that means under the default `SESSION_SCOPE=all`: a session started from *any* token, on *any* subdomain, opens this too — handing someone a token for one harness hands them the file tree. Set `SESSION_SCOPE=harness` if you ever need that separation. Treat it accordingly: it's exactly as sensitive as shell access to any other harness.
...
Claude Code and OpenCode run in the terminal's alternate-screen buffer (like `vim`/`htop`), which by spec has no scrollback of its own — neither tmux's `history-limit` nor xterm.js's local buffer apply there. For Claude Code that is only true of its fullscreen renderer: since 2.1 its default renderer is inline, writing the conversation into tmux's pane history, where PageUp does nothing and the wheel can't reach. So `harness-claude` sets `CLAUDE_CODE_NO_FLICKER=1`, plus `CLAUDE_CODE_DISABLE_MOUSE=1` so Claude never requests mouse reporting (tmux would pass that through and take native drag-select away). Instead, both apps manage their own scrollable transcript, driven by real key input (PageUp/PageDown). On wheel-scroll or touch-drag (only while on the alternate screen), `ttyd-kbfix.html` synthesizes a real PageUp/PageDown `KeyboardEvent` and dispatches it at `term.textarea` — the same element xterm.js itself listens on for physical typing, so it flows through the identical path a real keypress would. The wheel listener sits on the terminal's parent in the capture phase and stops the event there. Without that, xterm.js's own handler also fires: on a buffer with no scrollback it turns each notch into Up/Down arrow keys, which in Claude Code walk the prompt history while PageUp scrolls. (`term.input()` isn't exposed on this ttyd build's xterm.js — gated behind the "proposed API" flag — and `term.paste()` strips the ESC byte, so neither can carry a raw escape sequence; both were ruled out with a real headless-browser test before landing on the KeyboardEvent approach.) A plain shell prompt (normal buffer) is left alone, so xterm.js's default local scrollback still works there as-is.

On touch devices, a fixed bottom toolbar (Esc, Tab, Ctrl, Alt, Shift, Home, End, PgUp, PgDn, arrows — Termius/Termux-style) fills in keys a mobile on-screen keyboard doesn't have. Ctrl/Alt/Shift are one-shot sticky toggles: tap to arm, then the next keypress (physical or another toolbar tap) gets that modifier merged in and the toggle releases itself. `ttyd`'s page has no viewport meta tag, so a phone lays it out ~980px wide and shrinks it to the screen; the toolbar keys are sized in screen pixels instead, about 32px tall whatever the phone. The layout also follows the on-screen keyboard: when it opens, the toolbar sits just above it and the terminal refits to the space above the toolbar, so the prompt stays visible, and with the keyboard closed the toolbar no longer covers the terminal's last rows.
...
| `SESSION_IDLE_HOURS` | hours a session can go with no tab connected before just that session is closed, freeing its CLI's RAM — dynamic sessions on every harness, plus `main` on an `IDLE_EXEMPT` one. One value for all, or per-harness overrides as `24,opencode=3` (`0` disables) |
...
| The terminal window is fresh after a break, though the conversation came back | Expected: the container was idle-stopped (tmux dies with it) and the CLI relaunched with `--continue` | Nothing — that's the sleep/wake cycle. To skip it entirely for one harness, add it to `IDLE_EXEMPT` and pay the idle RAM |
...
| Every URL for one harness fails with `container harness-<type> does not exist` | Its container was removed while stopped — `docker container prune` / `docker system prune` deletes stopped containers, and an idle-stopped harness is one | The auth service rebuilds it from the spec it saved while the container was last seen. If that spec is gone too: `docker compose --profile on-demand up -d harness-<type>`. Putting the harness in `IDLE_EXEMPT` avoids the situation entirely |
