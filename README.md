...
| `IDLE_TIMEOUT_MIN` | minutes with no connected client and no model request from the harness before its container is stopped (`0` disables). One value for all, or per-harness overrides as `360,opencode=120`. A stop restarts every session in it, and slug sessions come back blank, so raise it if you leave them unattended |
