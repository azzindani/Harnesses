...
- **`harness-files` is a 12th `harness-*` container that isn't a CLI harness** — it's `dufs` (a small file-server), always-on rather than on-demand, giving web access to `project/`, `data/`, and `history/` at `files.<domain>` through the *same* JWT/cookie auth as everything else — writable in `data/` and `project/uploads`, read-only for the rest of `project/` and all of `history/` — enforced twice, by a dufs `--auth` rule (`@/data:rw,/project/uploads:rw,/`) *and* by keeping those two trees `:ro` bind mounts, so a browser session can't overwrite the `AGENTS.md`/`CLAUDE.md` that agents read as instructions. Uploads are live in every harness immediately (`/workspace/data`, `/workspace/uploads`). `--allow-archive` is on, so any directory zips and streams via `GET <dir>?zip` (and the UI's per-folder download button) — read-only, can't leave `/srv` with `--allow-symlink` off, so it doesn't reopen the write hole the `--auth` rule closed. There is no multi-select in dufs's UI; folder-level zip is the whole batch story. Note scripts must authenticate to this one with `?token=`+cookie, never `Authorization: Bearer`: dufs parses that header as *its* Digest credential and 401s a JWT after our own auth already passed. Because `data/` doubles as the sweep's exchange, `uploads` is in `run_all_checks.sh`'s `FIXTURES` allowlist and is a skipped path part in `verify_artifacts.py`: `data/uploads/` persists across rounds, anything loose in `data/` still gets cleared. It's recognized in `auth/server.py` via `UTILITY_SERVICES`, kept deliberately out of `HARNESSES`/`MULTI_INSTANCE_HARNESSES` so idle-sweep and dynamic-slug logic (which assume ttyd/tmux) leave it alone. See "File management" in `README.md`.
...
Full documented list lives in `.env.example` — don't let it drift from what `docker-compose.yml` actually reads. Highlights: `HARNESS_BASE_DOMAIN`, `JWT_SECRET`, `SESSION_TTL_DAYS` / `SESSION_REFRESH_DAYS` / `SESSION_SCOPE`, `IDLE_TIMEOUT_MIN` (per-harness too: `360,opencode=120`), `IDLE_EXEMPT`, `COLD_START_TIMEOUT_S`, `RETENTION_DAYS`, `SESSION_IDLE_HOURS` (closes one session with no tab for that long — a slug on any harness, plus `main` on an `IDLE_EXEMPT` one; `24,opencode=3` overrides one harness type), `MAX_INSTANCES_PER_HARNESS`, `TOKEN_<NAME>` (auto-filled by `auth` if blank), `FREE_FALLBACK` / `FREE_REQUIRE_TOOLS`, `OPENCODE_GO_API_KEY` / `OPENCODE_GO_CLIENTS`, `CLAUDE_MODEL` (Claude-only default model; exported as `ANTHROPIC_MODEL` by its entrypoint, so it beats a `/model` choice saved in `history/claude/settings.json` on every launch; it also fills the Opus and Haiku slots, so a session never touches a free OpenRouter model; append `[1m]` for a 1M-context model, or Claude Code assumes 200k and compacts early), `FOLIO_MCP_URL` / `FOLIO_MCP_TOKEN`, `PIPELINE_MCP_IMAGE` / `PIPELINE_PROJECT` (claude only: Pipeline's binary baked in at build time and run over stdio on `/workspace`; `PIPELINE_MCP_URL` / `_TOKEN` is the fallback, and that endpoint is read_only on its own filesystem, so it can neither write nor see `/workspace`), `WEB_MCP_URL`, and the 6 self-hosted `MCP_*` tool servers: `MATH_MCP_URL`/`_TOKEN`, `BROWSER_MCP_URL`/`_TOKEN`, `FS_MCP_URL`/`_TOKEN`, `ML_MCP_BASE_URL`/`_TOKEN`, `DATA_MCP_BASE_URL`/`_TOKEN`, `OFFICE_MCP_BASE_URL`/`_TOKEN`.
...
- **Tier repos** (`ml`, `data`, `office`, `docs`) mount several sub-servers at `<BASE_URL>/<sub-server>/mcp` and, since the tool trim, serve all of them as domain tools at `<BASE_URL>/mcp` (Data_Analyst: 8 tools there, 69 across its tiers). Their harness var is the BASE url (no `/mcp` suffix). `claude` and `opencode` register that one endpoint as `ml`/`data`/`office`/`docs`, and claude's entrypoint drops the old per-tier names from a restarted container's `~/.claude.json`; the other six entrypoints still loop the sub-server names and register each as `<repo>-<sub-server>` (e.g. `ml-basic`, `office-pptx-design`), which keep answering. `MCP_DISABLED` entries naming a sub-server (`office-xlsx-new`) no longer match anything in claude/opencode: disable the whole repo (`office`) instead.
...
- **Idle-stopping a container kills its tmux server**, so every session in it (base `main` + every dynamic slug) dies; only what the CLI wrote under `history/` survives. The answer is NOT to stop sweeping — the operator wants containers asleep (idle opencode ≈ 1.1GB + a few % CPU on a 15GB box, claude ≈ 460MB), and `IDLE_EXEMPT` is empty on purpose. Instead, `claude`'s and `opencode`'s entrypoints launch `main` with `--continue` (with a `|| <plain launch>` fallback for the first-ever boot), so waking resumes the conversation. Verified end to end: `docker stop` → visit → 200 in ~63s → the prior conversation is back in the pane. Do not add `--continue` to `HARNESS_LAUNCH_CMD` (dynamic slugs) — they share one `/workspace` and would all resume the same conversation. So slugs come back blank after a stop, and a phone drops its tab connection when locked: at 30 minutes a stop landed on an agent still working with no tab open. The operator's `.env` now uses `IDLE_TIMEOUT_MIN=720` (12h), and `_note_model_request` counts a harness's own proxy calls as use (by source IP, mapped each sweep). The operator wants claude genuinely stopped when idle, not standing by, with opening its link the only thing that starts it — so it stays out of `IDLE_EXEMPT` even though a stopped container is what `docker container prune` deletes (`_recreate_container` covers that). `IDLE_EXEMPT` holds only opencode, whose container also hosts a dev stack; its RAM comes back through `SESSION_IDLE_HOURS` instead.
...
- **Claude's harness needs >60s to boot** (its entrypoint registers each MCP server, ~26 before the one-endpoint change and ~10 after, before it execs ttyd), which is why `COLD_START_TIMEOUT_S` defaults to 120. Lowering it back to 30 makes the first visit after an idle-stop 504 on a container that was coming up fine.
...
| Waking a slept harness takes ~a minute | claude re-registers ~10 MCP servers before ttyd listens | Expected; `COLD_START_TIMEOUT_S=120` covers it. Trim `MCP_*` vars to speed it up |
...
| Every URL for one harness 502s, `container harness-<type> does not exist` | The container was removed while it was stopped: `docker container prune` / `docker system prune` deletes stopped containers and leaves running ones, which is exactly what an idle-stopped harness is. It took harness-claude out overnight on 2026-09-18 | `/verify` rebuilds it from `/data/container-specs/<name>.json`, written whenever the container is seen (`_save_container_spec` → `_recreate_container`). With no spec: `docker compose --profile on-demand up -d harness-<type>`, then put each pane back on its conversation. Listing a harness in `IDLE_EXEMPT` avoids the hazard: its sessions close to free RAM (`SESSION_IDLE_HOURS`) and the container stays |
| A harness's `main` terminal can't reconnect while its slug tabs work | `main`'s tmux session is gone — the CLI in it was quit, or `_close_idle_main` stopped it to free RAM — and ttyd can only run `tmux attach-session -t main` | `/verify` recreates it (`_ensure_base_session`) from `/run/main-launch`, the launch line each entrypoint writes at boot. A container started from an image predating that file has none: write it (or recreate the session) by hand, no restart needed |
| `<harness>-<slug>` URL 429s | `MAX_INSTANCES_PER_HARNESS` reached | Let an idle slug time out, or raise the cap |
| Container never sleeps | `IDLE_TIMEOUT_MIN=0`, a client still holds an open websocket, or an agent in it is still calling a model | Idle sweep checks for a real TCP connection, not just `/verify` timestamps, and the proxy counts each model request from a harness as use |
...
| Claude Code's `est $` or `/usage` cost is far above what OpenCode Go charges | It prices ids it doesn't recognize at list rates, and cached tokens used to reach it as fresh input | The claude entrypoint writes `modelPricing` from models.dev into `/etc/claude-code/managed-settings.json` (managed settings are the only place that key works), and the proxy reports `cached_tokens` as `cache_read_input_tokens` |
| Statusline still says `/200k` after adding `[1m]` to `CLAUDE_MODEL` | The running `claude`, and the tmux pane shell it was typed into, kept the old `ANTHROPIC_MODEL` | Restart the harness, or `tmux respawn-pane -k` the pane (the new shell reads tmux's global env) and `claude --resume <sessionId>` — the id is in `history/claude/sessions/<pid>.json` |
| Claude Code sits on "Waiting for API response · will retry in 4m · check your network" | That banner is Claude Code's quiet-stream warning, not an error; it gives up at 5 minutes. Two things made streams quiet. A reasoning model can think silently for a minute or more (76s seen on Muse Spark at 200k context), and the proxy forwarded nothing meanwhile. And a call on a free OpenRouter model can hang for minutes and then 504: built-in `Explore` subagents are capped at the Opus slot whenever the main model isn't a Claude model, and titles and WebFetch summaries use Haiku | The proxy asks Go for reasoning summaries and streams them as thinking blocks, and pings every 10s on a quiet stream (`_with_keepalive`). With `CLAUDE_MODEL` set, every slot uses it, and `harness-claude` sets `CLAUDE_CODE_DISABLE_EXPLORE_INHERIT_CAP=1` so `Explore` inherits the main model. A long pause with the timer stopped is a tool running (ctrl+b twice backgrounds it). A long "thinking" is mostly context size and the task. `/effort` does reach Go as `reasoning.effort`, the same knob as opencode's variant picker, but on one test question medium, high and xhigh all gave first text in 11–14s (median of 3). A Go stream silent for 120s is cut. If no text or tool call has gone out yet, the proxy asks Go again unstreamed and sends that answer down the same stream (`_stream_or_fallback`; three 120s hangs in a row at ~700k tokens cost ten minutes before it). Otherwise the stream ends in a retryable `overloaded_error`. Separately, Go answers 429 "Output token rate limit exceeded" when two large sessions generate at once, and Claude Code backs off |
| Claude Code's built-in `WebSearch` returns no results | Anthropic runs that search on its own servers, and the proxy doesn't translate it for Go or OpenRouter | Denied outright now, in the `permissions.deny` the claude entrypoint writes into managed settings, so a session isn't offered a tool that burns a turn on an empty result. Search is the web MCP's `search`. `WebFetch` is deliberately NOT denied: it fetches in the client and summarises with the configured model, and was verified working on example.com and docs.python.org |
