...
| Claude Code's built-in `WebSearch` returns no results | Anthropic runs that search on its own servers, and the proxy doesn't translate it for Go or OpenRouter | Use the web MCP's `search` / `fetch_content` |
