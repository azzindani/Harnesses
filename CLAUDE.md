...
- **Idle-stopping a container destroys live work, not just RAM.** `docker stop` kills the container's tmux server, so every session in it (base `main` + every dynamic slug) is gone; only what the CLI itself wrote under `history/` survives. That's why `IDLE_EXEMPT` exists and why `claude,opencode` (the daily drivers) are in it — don't "tidy it up" back to sweeping everything.
